<script lang="ts">
  ...
  let filteredEducationHistory = $derived(
    [...data.eductation].sort(dateFilters[educationFilterIdx]),
  );
...
<h2 class="small-padding center-align bottom-shadow responsive border">
  Work History
</h2>
    ...
    <table class="center-align border stripes surface-container">
      <thead class="surface-container-lowest square fixed">
        <tr>
          ...
          <th>
            <button class="transparent" onclick={toggleEmploymentEndDateFilter}>
              Ended
      ...
      <tbody>
        {#each filteredEmploymentHistory as work}
          <tr>
            <td
              >{work.startDate.toLocaleString("en-US", { year: "numeric" })}</td
            >
...
<h2 class="small-padding round center-align bottom-shadow responsive border">
  Education
</h2>
<article class="surface-container-lowest center-align responsive border">
    ...
    <table class="center-align border stripes surface-container">
      ...
      <tbody>
        {#each data.certification as cert}
          <tr>
            <td
              >{cert.startDate.toLocaleString("en-US", { year: "numeric" })}</td
            >
